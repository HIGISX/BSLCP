...
class ImprovedGeneticAlgorithm(GeneticAlgorithm):
    """
    改进版遗传算法：
    - 自适应变异概率：随迭代进度线性衰减，并保留下限 0.05
    - 精英保留：每代保留若干最优个体直接进入下一代
    """

    def __init__(self, *args, elite_num=2, **kwargs):
        super().__init__(*args, **kwargs)
        self.elite_num = elite_num

    def mutation(self, chromosome):
        """自适应变异：在原地对染色体执行变异，保证与基类兼容。"""
        progress = self.current_iteration / max(self.iterations, 1)
        adaptive_mutation_prob = self.mutation_prob * (1 - progress) + 0.05
    ...
    def create_generation(self, for_reproduction):
        """精英保留 + 生成新一代。"""
        new_generation = []
        elites = sorted(for_reproduction, key=lambda c: c.fitness)[:self.elite_num]
        target_size = max(self.generation_size - self.elite_num, 0)
        while len(new_generation) < target_size:
            ...
            if child1 is not None:
                self.mutation(child1)
                new_generation.append(Chromosome(child1, self.fitness(child1)))
            if child2 is not None and len(new_generation) < target_size:
                self.mutation(child2)
                new_generation.append(Chromosome(child2, self.fitness(child2)))
        # 保留精英（深拷贝防止后续修改）
        new_generation.extend(copy.deepcopy(elites))
        return new_generation